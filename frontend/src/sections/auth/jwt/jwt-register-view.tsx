...
export default function JwtRegisterView() {
  ...
  const RegisterSchema = Yup.object().shape({
    ...
    typeUser: Yup.string().required('Please select user type'),
  });
  ...
  const defaultValues = {
    ...
    password: '',
    password2: '',
    typeUser: USER_TYPES.ADS_CLIENT,
  };
  ...
  const onSubmit = useCallback(
    async (data: FormValuesProps) => {
      try {
        await register?.(data.email, data.password, data.typeUser, data.password2);
        ...
        setErrorMsg(String(message).slice(0, 200)); 
      }
    },
    [register, reset, returnTo, router]
  );
